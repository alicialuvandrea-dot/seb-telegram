...
async def exec_action(action_type: str, payload: dict, *, chat_id: int | None = None, bot=None) -> str:
    if action_type == "save_memory":
        ...
        is_chinese_voice = voice_key in config.MINIMAX_CHINESE_VOICES
        try:
            ...
            if zh_text and not is_chinese_voice:
...
- mandarin：普通话，想用中文说话时使用
- cantonese：粤语，想说粤语时使用
- korean：韩语，聊到韩流/kpop/韩剧等韩国话题时优先使用
...
zh 字段说明：
- 选 mandarin 或 cantonese 时不填 zh（声音本身已是中文，不需要配文）
- 其他声音填中文正文，语音后紧接着发出
